...
const ContactConfirmationEmail = ({ name }: ContactConfirmationProps) => (
  <Html lang="hu" dir="ltr">
    <Head />
    <Preview>Üzeneted megérkezett — {SITE_NAME}</Preview>
    <Body style={main}>
      <Container style={wrapper}>
        ...
        <Container style={container}>
          <Heading style={h1}>
            {name ? `Köszönjük, ${name}!` : 'Köszönjük az üzeneted!'}
          </Heading>
          <Text style={text}>
            Megkaptuk a megkeresésed és csapatunk hamarosan válaszol. Általában <strong>24 órán belül</strong> visszajelzünk.
          </Text>
          <Hr style={hr} />
          <Text style={textSmall}>
            Ha sürgős a kérdésed, keress minket közvetlenül a közösségi média felületeinken.
          </Text>
        </Container>
        <Section style={footerSection}>
          <Text style={footerBrand}>{SITE_NAME}</Text>
          <Text style={footerText}>Streetwear amit érzel. ■</Text>
        </Section>
      </Container>
    </Body>
  </Html>
...
const footerText = { color: '#666666', fontSize: '11px', margin: '0', letterSpacing: '0.05em' }
