...
A KYC-eljárás során kezelt adatok körét, célját, jogalapját (szerződés teljesítése és jogi kötelezettség, Pmt.), megőrzési idejét és az érintetti jogokat a mindenkori KYC Adatkezelési Tájékoztató tartalmazza, amely a Partner Központban elérhető.$body$,
    ...
    INSERT INTO public.partner_contracts (
      ...
      NEW.user_id, NEW.id, v_contract_number, 'v2.2', v_body,
...
-- Frissítjük a még aláírásra váró szerződések szövegét és verzióját, hogy az új modellt lássák.
UPDATE public.partner_contracts
SET contract_version = 'v2.2',
    contract_body = replace(
      replace(
        replace(contract_body,
...
WHERE status = 'pending_partner_signature'
  AND (contract_version IS DISTINCT FROM 'v2.2' OR contract_body ILIKE '%%v2.1%%' OR contract_body ILIKE '%%50 000 Ft + FELFELÉ%%');